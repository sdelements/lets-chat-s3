...
S3.prototype.getUrl = function(file) {
    ...
    var endpoint = this.options.endpoint || 's3-' + this.options.region + '.amazonaws.com';

    if (this.options.style === 'path') {
      return 'https://' + endpoint + '/' + this.options.bucket + '/' + filePath;
    }
    else {
      return 'https://' + this.options.bucket + '.' + endpoint + '/' + filePath;
    }
};
